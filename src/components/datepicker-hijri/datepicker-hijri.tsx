...
export class DatepickerHijri {
  ...
  @Prop({ reflect: true }) placeholder: string = "Please Select Date";
  @Prop({ reflect: true }) autoSetSelectedDate: boolean = false;
  ...
  handleOnClickOutSide(e) {
    if (
      !this.hasSomeParentId(e.target, this.calenderContainerId) &&
      !(e.target.id == this.reference)
    )
  ...
  setSelectedDate = (selectedDate, triggerEvent = true) => {
    ...
    if (triggerEvent) reference.dispatchEvent(new Event("change"));

    if (this.onDateSelectClose) this.displayCalender = false;
  };
  ...
  componentWillLoad() {
    ...
    if (!moment(this.selectedDate, this.dateFormat)._isValid) {
      ...
      if (
        !moment(
          document.querySelector("#" + this.reference).getAttribute("value"),
          this.dateFormat
        )._isValid
      ) {
        selectedDate = moment().format(this.dateFormat);
      } else {
        selectedDate = document
          .querySelector("#" + this.reference)
          .getAttribute("value");
      }
      var reference = document.querySelector("#" + this.reference);
      reference.setAttribute("placeholder", this.placeholder);
      this.selectedDate = selectedDate;

      if (this.autoSetSelectedDate) {
  ...
  render() {
    return (
      <div
        id={this.calenderContainerId}
        style={{
          visibility: this.displayCalender ? "visible" : "hidden",
          "z-index": "99999",
          margin: "5px",
        }}
      >
